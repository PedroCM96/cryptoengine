...
export  class MoveCharacter extends Action {
    ...
    protected async doExecute(global: Global): Promise<void> {
        ...
        for (const movement of this.movements.slice(this.moveToExecIndex || 0)) {
            ...
            this.moveToExecIndex === null ? this.moveToExecIndex = 1 : this.moveToExecIndex++;
        }
    ...
    static load(data: any): Action {
        ...
        if (!movements) {
            throw new Error(`MoveCharacter load method should receive and object with a key movements`);
        }

        if(!(movements instanceof Array)) {
            throw new Error(`MoveCharacter load method: key movements should be an array of at least one movement`);
        }
