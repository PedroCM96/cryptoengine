...
export class Map {
    ...
    constructor(private readonly id: number, private readonly data: MapData, private img: HTMLImageElement) {
    ...
    restoreInteractiveEvents(): void {
        for (const event of this.data.events.values()) {
            if (event.isPermanent() && !event.isRunning() && event.getTrigger() === Trigger.INTERACT) {
                event.restore();
            }
        }
    }
    ...
    restoreCollideEvents(): void {
        for (const event of this.data.events.values()) {
            if (event.isPermanent() && !event.isRunning() && event.getTrigger() === Trigger.COLLISION) {
                event.restore();
            }
        }
    }
    ...
    private renderNpcs(ctx: CanvasRenderingContext2D, referencePosition: Position): void {
        for (const e of this.data.events.values()) {
            ...
            if (this.camera.isVisible(npc.getPosition(), referencePosition)) {
                npc.render(ctx, {tlp: this.camera.getTopLeftCornerCellPosition(referencePosition)});
    ...
    private handleNpcMovement(event: Event, referencePosition: Position) {
        ...
        const shouldMove = Math.floor(Math.random() * 10000) > 9900 && !event.isRunning();
